...
static std::vector<std::unordered_set<std::string>> block_buckets(BUCKET_COUNT);

static inline size_t host_hash(const std::string& host) { // 값을 참조만 해서 (복사 x) 좀 더 빠르다고 한다.
    return std::hash<std::string>{}(host) % BUCKET_COUNT; // 16개 버킷 중 하나를 리턴하도록 설정.
}

static bool load_block_list(const char* filename) {
    ...
    while (std::getline(fin, line)) {
        ...
        // "순번<TAB or ,>도메인" 파싱
        std::string domain;
        size_t pos = line.find(','); // CSV 파일은 구분으로 ,를 쓰니까 ,를 찾아서 pos에 저장
        
        if (pos != std::string::npos) { // 구분자가 있으면
            domain = line.substr(pos + 1); // domain == 구분자 다음부터 끝까지
        } else {
            domain = line;  // 구분자 없으면 전체를 도메인으로 -> 과제 파일에서는 없는걸로 보임
        }

        if (domain.empty()) continue; // 만약 도메인 x이면 continue

        size_t bucket = host_hash(domain); // hash하고 %16해서 bucket 번호 구하는 함수 사용
        block_buckets[bucket].insert(domain); // block_buckets는 vector니까 bucket 번호에 해당하는 unordered_set<string>에 domain 삽입
    }

	auto end = std::chrono::high_resolution_clock::now();
	auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);

	printf("Loaded domains in %lld ms\n", ms.count());

    for (int i = 0; i < BUCKET_COUNT; i++) {
        printf("  Bucket[%2d]: %zu entries\n", i, block_buckets[i].size()); // 각 버킷에 몇 개의 도메인이 있는지 출력
    }
    return true;
}
...
static int check_host(unsigned char *data, int len){
	...
	if (colon != std::string::npos) {
		host = host.substr(0, colon); // 포트 제거
	}

	size_t bucket = host_hash(host); // 저장 버킷 계산(host의)

	auto start = std::chrono::high_resolution_clock::now();

	bool is_blocked = block_buckets[bucket].find(host) != block_buckets[bucket].end();

	bool found = (block_buckets[bucket].find(host) != block_buckets[bucket].end());
	auto end = std::chrono::high_resolution_clock::now();
	
	auto ns = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start);
	printf("Searching time: %lld ns\n", ns.count());
	
	if (is_blocked) { // 버킷에서 host 찾기 못찾으면 end 반환
...
int main(int argc, char **argv)
{
	...
	if (argc == 2) {
        if (!load_block_list(argv[1])) { // excel 파일 받아서 확인함.
            exit(EXIT_FAILURE);
		}
	}
    else {
